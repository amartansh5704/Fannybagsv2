    ...
    <KhapeetarLayout>
      ...
      <div style={{ minHeight:'100vh', background:'#06060a', color:'#fff', position:'relative', overflow:'hidden', fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif' }}>
        ...
        <div style={{ position:'relative', zIndex:1, padding:'28px 32px 60px', maxWidth:1100, margin:'0 auto' }}>
          ...
          {/* Balance hero */}
          <div style={{ position:'relative', overflow:'hidden', background:'linear-gradient(135deg,rgba(16,185,129,0.12),rgba(20,184,166,0.07),rgba(255,255,255,0.02))', border:'1px solid rgba(16,185,129,0.18)', borderRadius:24, padding:'36px', marginBottom:24, animation:'kFadeInUp 0.4s ease-out' }}>
          ...
          {/* Stats */}
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))', gap:14, marginBottom:28, animation:'kFadeInUp 0.4s ease-out 0.1s both' }}>
            {[
            ...
            <div style={{ marginBottom:28, animation:'kFadeInUp 0.4s ease-out 0.15s both' }}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                {depositReqs.map((req, idx) => {
                  ...
                  return (
                    <div key={req.id} style={{ display:'flex', alignItems:'center', gap:14, padding:'14px 18px', background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:14, animation:`kFadeInUp .3s ease-out ${idx * 0.04}s both` }}>
                      ...
                      {req.screenshotUrl && <a href={req.screenshotUrl} target="_blank" rel="noreferrer" style={{ fontSize:11, color:'#34d399', textDecoration:'none', fontWeight:600 }}>View SS ↗</a>}
                ...
                {filteredTx.map((tx, idx) => {
                  ...
                  return (
                    <div key={tx.id}
                      ...
                      style={{ display:'flex', alignItems:'center', gap:14, padding:'16px 20px', background: isH ? 'rgba(255,255,255,0.05)' : 'rgba(255,255,255,0.03)', border:`1px solid ${isH ? s.border : 'rgba(255,255,255,0.05)'}`, borderRadius:16, transition:'all .25s ease', animation:`kFadeInUp .3s ease-out ${idx * 0.03}s both`, transform: isH ? 'translateX(3px)' : 'translateX(0)' }}>
