...
export default function KhapeetarWalletPage() {
  ...
  const filteredTx = transactions.filter(tx => {
    ...
    if (activeTab === 'credit') return ['deposit','credit','received','royalty','refund'].includes(t)
    if (activeTab === 'debit')  return ['withdrawal','debit','escrow'].includes(t)
    return true
  })
    ...
    return (
      <KhapeetarLayout>
        ...
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', minHeight:'100vh', background:'#06060a' }}>
          <Loader2 style={{ animation:'kSpin 1s linear infinite', color:'#10b981', width:32, height:32 }} />
  ...
  return (
    <KhapeetarLayout>
      <style jsx global>{`
        ...
        @keyframes kFadeInUp   { from{opacity:0;transform:translateY(14px)} to{opacity:1;transform:translateY(0)} }
        @keyframes kFadeInDown { from{opacity:0;transform:translateY(-10px)} to{opacity:1;transform:translateY(0)} }
        @keyframes kGradShift  { 0%{background-position:0% 50%} 50%{background-position:100% 50%} 100%{background-position:0% 50%} }
        @keyframes kPulseDot   { 0%,100%{opacity:1;transform:scale(1)} 50%{opacity:.4;transform:scale(1.4)} }
        @keyframes kPop        { 0%{transform:scale(.9);opacity:0} 60%{transform:scale(1.05)} 100%{transform:scale(1);opacity:1} }

        dialog::backdrop { background:rgba(0,0,0,0.85); backdrop-filter:blur(14px); }
        dialog { border:none; padding:0; background:transparent; }
        dialog[open] { animation:kFadeInUp 0.3s ease-out; }

        .k-input::placeholder { color:#3f3f46; }
        .k-input:focus { outline:none; border-color:rgba(16,185,129,0.4); box-shadow:0 0 0 3px rgba(16,185,129,0.08); }
        .k-tx-scroll::-webkit-scrollbar { width:4px; }
        .k-tx-scroll::-webkit-scrollbar-thumb { background:rgba(255,255,255,0.06); border-radius:2px; }
      ...
      <div style={{ minHeight:'100vh', background:'#06060a', color:'#fff', position:'relative', overflow:'hidden', fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif' }}>
        ...
        <div style={{ position:'fixed', inset:0, pointerEvents:'none', overflow:'hidden', zIndex:0 }}>
          <div style={{ position:'absolute', top:'-80px', right:'-40px', width:'500px', height:'500px', background:'radial-gradient(circle,rgba(16,185,129,0.07) 0%,transparent 70%)', borderRadius:'50%' }} />
          <div style={{ position:'absolute', bottom:'-60px', left:'-60px', width:'400px', height:'400px', background:'radial-gradient(circle,rgba(20,184,166,0.05) 0%,transparent 70%)', borderRadius:'50%' }} />
        ...
        {/* Header */}
        <div style={{ position:'relative', zIndex:1, borderBottom:'1px solid rgba(255,255,255,0.05)', background:'rgba(6,6,10,0.8)', backdropFilter:'blur(20px)', animation:'kFadeInDown 0.4s ease-out' }}>
          <div style={{ padding:'16px', maxWidth:1100, margin:'0 auto', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:14 }}>
            <div style={{ display:'flex', alignItems:'center', gap:14 }}>
              ...
              <div>
                <h1 style={{ fontSize:22, fontWeight:800, margin:0, background:'linear-gradient(135deg,#fff 0%,#a1a1aa 100%)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent' }}>Wallet</h1>
                <p style={{ fontSize:13, color:'#52525b', margin:'2px 0 0 0', fontWeight:500 }}>Manage your funds</p>
              </div>
            </div>
            <button onClick={() => loadAll(true)} disabled={refreshing}
              ...
              <RefreshCw size={13} style={{ animation: refreshing ? 'kSpin 1s linear infinite' : 'none' }} /> Refresh
            </button>
          </div>
        </div>
        ...
        <div style={{ position:'relative', zIndex:1, padding:'20px 16px 60px', maxWidth:1100, margin:'0 auto' }}>
          ...
          <div style={{ position:'relative', overflow:'hidden', background:'linear-gradient(135deg,rgba(16,185,129,0.12),rgba(20,184,166,0.07),rgba(255,255,255,0.02))', border:'1px solid rgba(16,185,129,0.18)', borderRadius:24, padding:'28px 20px', marginBottom:24, animation:'kFadeInUp 0.4s ease-out' }}>
            <div style={{ position:'absolute', top:0, left:0, right:0, height:2, background:'linear-gradient(90deg,transparent,#10b981,#14b8a6,transparent)', backgroundSize:'200% 100%', animation:'kGradShift 4s ease-in-out infinite' }} />
            <div style={{ position:'absolute', top:'-30px', right:'-30px', width:200, height:200, background:'radial-gradient(circle,rgba(16,185,129,0.15) 0%,transparent 70%)', borderRadius:'50%', pointerEvents:'none' }} />
            ...
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:20, position:'relative' }}>
              ...
              <div style={{ minWidth:0 }}>
                <p style={{ fontSize:11, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.12em', color:'#059669', margin:'0 0 8px 0' }}>Available Balance</p>
                <p style={{ fontSize:'clamp(28px, 7vw, 48px)', fontWeight:800, color:'#fff', margin:0, lineHeight:1.1, letterSpacing:'-0.02em', wordBreak:'break-all' }}>
                  ₹{balance.toLocaleString('en-IN')}
                </p>
                <div style={{ display:'flex', alignItems:'center', gap:6, marginTop:10 }}>
                  <div style={{ width:7, height:7, borderRadius:'50%', background:'#34d399', animation:'kPulseDot 2s ease-in-out infinite' }} />
          ...
          {/* Stats */}
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))', gap:12, marginBottom:28, animation:'kFadeInUp 0.4s ease-out 0.1s both' }}>
            {[
          ...
          {depositReqs.length > 0 && (
            <div style={{ marginBottom:28, animation:'kFadeInUp 0.4s ease-out 0.15s both' }}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:8 }}>
                {depositReqs.map((req, idx) => {
                  ...
                  return (
                    <div key={req.id} style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 14px', background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:14, flexWrap:'wrap', animation:`kFadeInUp .3s ease-out ${idx * 0.04}s both` }}>
                      ...
                      <div style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
                        {req.screenshotUrl && <a href={req.screenshotUrl} target="_blank" rel="noreferrer" style={{ fontSize:11, color:'#34d399', textDecoration:'none', fontWeight:600 }}>View ↗</a>}
          ...
          {withdrawReqs.length > 0 && (
            <div style={{ marginBottom:28, animation:'kFadeInUp 0.4s ease-out 0.18s both' }}>
          ...
          {/* Transaction history */}
          <div style={{ animation:'kFadeInUp 0.4s ease-out 0.2s both' }}>
            ...
            {filteredTx.length === 0 ? (
              ...
              <div className="k-tx-scroll" style={{ display:'flex', flexDirection:'column', gap:8 }}>
                {filteredTx.map((tx, idx) => {
                  ...
                  return (
                    <div key={tx.id}
                      ...
                      style={{ display:'flex', alignItems:'center', gap:12, padding:'14px 16px', background: isH ? 'rgba(255,255,255,0.05)' : 'rgba(255,255,255,0.03)', border:`1px solid ${isH ? s.border : 'rgba(255,255,255,0.05)'}`, borderRadius:16, transition:'all .25s ease', animation:`kFadeInUp .3s ease-out ${idx * 0.03}s both`, transform: isH ? 'translateX(3px)' : 'translateX(0)', flexWrap:'wrap' }}>
      ...
      <dialog ref={depositDialogRef} style={{ width:480, maxWidth:'95vw', borderRadius:24, overflow:'hidden', boxShadow:'0 25px 80px rgba(0,0,0,0.7)' }}>
        <div style={{ background:'#0f0f14', border:'1px solid rgba(255,255,255,0.08)', borderRadius:24, overflow:'hidden' }}>
          ...
          <div style={{ padding:24, maxHeight:'65vh', overflowY:'auto' }} className="k-tx-scroll">
            {submitDone ? (
              <div style={{ textAlign:'center', padding:'40px 0', animation:'kPop .4s ease-out' }}>
              ...
              <div>
                ...
                <input type="number" placeholder="e.g. 1000" value={depositAmt} onChange={e => setDepositAmt(e.target.value)} className="k-input"
              ...
              <div>
                ...
                <input placeholder="e.g. 425612345678" value={utrNumber} onChange={e => setUtrNumber(e.target.value)} className="k-input"
                ...
                {ssUrl ? (
                  ...
                  <div onClick={() => ssInputRef.current?.click()}
                    ...
                    onMouseEnter={e => { if (!ssUploading) e.currentTarget.style.borderColor='rgba(16,185,129,0.3)' }}
                    onMouseLeave={e => { e.currentTarget.style.borderColor='rgba(255,255,255,0.12)' }}>
                    {ssUploading ? <Loader2 size={22} style={{ animation:'kSpin 1s linear infinite', color:'#34d399' }} /> : <Upload size={22} color="#52525b" />}
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
                  ...
                  <button onClick={submitDepositRequest} disabled={submitting || ssUploading}
                    ...
                    {submitting ? <><Loader2 size={14} style={{ animation:'kSpin 1s linear infinite' }} /> Submitting...</> : '✓ Submit Request'}
                  </button>
                </div>
              </div>
            )}
      ...
      <dialog ref={payoutDialogRef} style={{ width:480, maxWidth:'95vw', borderRadius:24, overflow:'hidden', boxShadow:'0 25px 80px rgba(0,0,0,0.6)' }}>
        <div style={{ background:'#0f0f14', border:'1px solid rgba(255,255,255,0.08)', borderRadius:24, overflow:'hidden' }}>
          ...
          <div style={{ padding:26, maxHeight:'70vh', overflowY:'auto' }} className="k-tx-scroll">
            {payoutDone ? (
              <div style={{ textAlign:'center', padding:'40px 0', animation:'kPop .4s ease-out' }}>
              ...
              <>
                ...
                <div style={{ background:'rgba(59,130,246,0.05)', border:'1px solid rgba(59,130,246,0.12)', borderRadius:12, padding:'12px 16px', marginBottom:18 }}>
                  <p style={{ fontSize:12, color:'#60a5fa', margin:0, lineHeight:1.7 }}>ℹ️ <strong>How it works:</strong> Submit request → Admin sends money manually → Admin marks as paid → Your wallet is debited.</p>
                </div>
                ...
                <input type="number" placeholder="Min ₹100" value={payoutAmt} onChange={e => setPayoutAmt(e.target.value)} className="k-input"
                ...
                <input placeholder={payoutMethod === 'upi' ? 'yourname@upi' : 'Account No · IFSC · Bank Name'}
                  value={payoutDetail} onChange={e => setPayoutDetail(e.target.value)} className="k-input"
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
                  ...
                  <button onClick={submitWithdrawal} disabled={payingOut}
                    ...
                    {payingOut ? <><Loader2 size={14} style={{ animation:'kSpin 1s linear infinite' }} /> Submitting...</> : '🏦 Request Withdrawal'}
                  </button>
                </div>
              </>
            )}
