...
<div class="page">
  ...
  <div class="section">
    ...
    <div class="parties-grid">
      <div class="party-box"><div class="party-role">🎤 Artist (Licensor)</div><div class="party-name">${artist.name}</div><div class="party-email">${artist.email}</div></div>
      <div class="party-box"><div class="party-role">💎 Fan Investor (Licensee)</div><div class="party-name">${fan.name}</div><div class="party-email">${fan.email}</div></div>
    </div>
  </div>
  <div class="section">
  ...
  <div class="section"><div class="section-title">Recitals</div><p>WHEREAS, the Artist wishes to raise funds to produce, distribute, and promote the musical work titled <strong>"${s.title}"</strong> (the "Song");</p><p>WHEREAS, the Fan Investor ("Investor") wishes to participate financially in the Song's campaign and receive a proportional share of future streaming revenue generated by the Song;</p><p>NOW, THEREFORE, in consideration of the mutual covenants set forth herein and for other good and valuable consideration, the parties agree as follows:</p></div>
  <div class="section clauses"><div class="section-title">Terms and Conditions</div><ol><li><strong>Grant of Revenue Participation.</strong> In exchange for the Investment Amount of ₹${investment.amount.toLocaleString('en-IN')}, the Artist grants the Investor a revenue participation interest of <strong>${investment.ownershipPct.toFixed(4)}%</strong> of the Song's net streaming revenue.</li><li><strong>Revenue Pool.</strong> The total fan revenue pool for this Song is <strong>${campaign.fanRevenueShare}%</strong> of all net streaming revenue.</li><li><strong>Breakeven.</strong> Based on a platform streaming rate of ₹${economics.revenuePerStream.toFixed(2)} per stream, the Song must reach approximately <strong>${economics.breakevenStreamsFormatted} streams</strong> for the collective fan pool to recover the total investment.</li><li><strong>Payment Terms.</strong> Revenue distributions will be processed periodically by FannyBags in accordance with its royalty distribution schedule.</li><li><strong>Non-Transferability.</strong> This revenue participation interest is personal to the Investor and may not be transferred or assigned without express written consent.</li><li><strong>Artist's Retained Rights.</strong> The Artist retains full creative control, copyright ownership, and master rights. This agreement does not confer any equity or decision-making rights upon the Investor.</li><li><strong>No Guaranteed Returns.</strong> Returns are contingent on the Song's commercial performance.</li><li><strong>Platform Role.</strong> FannyBags acts solely as a facilitating platform.</li><li><strong>Duration.</strong> This agreement remains in effect for as long as the Song generates streaming revenue.</li><li><strong>Governing Law.</strong> This agreement shall be governed by the laws of India.</li></ol></div>
  <div class="signature-grid"><div class="sig-box"><div class="sig-label">Artist (Licensor)</div><div class="sig-name">${artist.name}</div><div class="sig-role">Artist · ${dateStr}</div></div><div class="sig-box"><div class="sig-label">Fan Investor (Licensee)</div><div class="sig-name">${fan.name}</div><div class="sig-role">Investor · ${dateStr}</div></div></div>
  <div class="footer"><p>This agreement was generated by FannyBags · Contract ID: ${investmentId} · ${dateStr}</p></div>
</div>
</body>
</html>`
            ...
            <div style={{ animation:'sdFadeInRight 0.6s ease-out', display:'flex', flexDirection:'column', gap:'20px' }}>
              ...
              {fanRevenueShare > 0 && (
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', background:'linear-gradient(135deg,rgba(168,85,247,0.08),rgba(236,72,153,0.04))', border:'1px solid rgba(168,85,247,0.15)', borderRadius:'16px', padding:'16px 20px', animation:'sdFadeInStagger 0.5s ease-out 0.5s both' }}>
                  ...
                  <span style={{ fontSize:'20px', fontWeight:800, color:'#d8b4fe', letterSpacing:'-0.02em' }}>
            ...
            <div style={{ padding:'24px 28px', overflowY:'auto', flex:1 }}>
              ...
              <div style={{ marginBottom:'16px' }}>
                ...
                <div style={{ position:'relative' }}>
                  ...
                  <input type="number" value={amount}
                    ...
                    placeholder="0"
                    style={{ width:'100%', background: focused==='invest-amount' ? 'rgba(255,255,255,0.06)' : 'rgba(255,255,255,0.03)', border:`1px solid ${focused==='invest-amount' ? 'rgba(236,72,153,0.5)' : 'rgba(255,255,255,0.07)'}`, borderRadius:'14px', padding:'16px 18px 16px 38px', color:'#fff', fontSize:'18px', fontWeight:700, outline:'none', transition:'all 0.3s ease', fontFamily:'inherit', boxSizing:'border-box', boxShadow: focused==='invest-amount' ? '0 0 0 3px rgba(236,72,153,0.08)' : 'none' }}
                  />
                </div>
              </div>
              ...
              <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px' }}>
                ...
                <button onClick={invest} disabled={insufficient || investing}
                  ...
                  style={{ position:'relative', padding:'16px', borderRadius:'14px', background:(insufficient||investing) ? 'rgba(255,255,255,0.03)' : 'linear-gradient(135deg,#ec4899,#f43f5e)', border:'none', color:(insufficient||investing) ? '#3f3f46' : '#fff', fontSize:'14px', fontWeight:700, cursor:(insufficient||investing) ? 'not-allowed' : 'pointer', transition:'all 0.3s ease', fontFamily:'inherit', opacity:(insufficient||investing) ? 0.4 : 1, overflow:'hidden' }}>
                  {investing ? (
            ...
            <div style={{ padding:'28px', textAlign:'center' }}>
              ...
              <div style={{ display:'flex', flexDirection:'column', gap:10, marginBottom:28, textAlign:'left' }}>
                {[
                  ...
                  { icon:'📄', text:'Receive a legal royalty contract' },
                  { icon:'💰', text:'Track earnings in real time' },
                ].map((p, i) => (
                  <div key={i} style={{ display:'flex', alignItems:'center', gap:12, padding:'12px 16px', background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:12 }}>
                    <span style={{ fontSize:18 }}>{p.icon}</span>
                    <span style={{ fontSize:13, color:'#a1a1aa', fontWeight:500 }}>{p.text}</span>
                  </div>
                ))}
              </div>
