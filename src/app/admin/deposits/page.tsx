...
const STATUS = {
  ...
  approved: { color: '#34d399', bg: 'rgba(16,185,129,0.1)',  border: 'rgba(16,185,129,0.2)',  label: 'Approved' },
  ...
  return (
    <AdminLayout>
      <style jsx global>{`
        ...
        @keyframes dFadeInUp  { from{opacity:0;transform:translateY(12px)} to{opacity:1;transform:translateY(0)} }
        @keyframes dLightbox  { from{opacity:0;transform:scale(.95)} to{opacity:1;transform:scale(1)} }
        @keyframes dPulseDot  { 0%,100%{opacity:1} 50%{opacity:.3} }
        .d-input::placeholder { color:#3f3f46; }
        .d-input:focus { outline:none; border-color:rgba(239,68,68,0.4); box-shadow:0 0 0 3px rgba(239,68,68,0.08); }
      `}</style>

      {/* Lightbox */}
      {lightbox && (
        <div onClick={() => setLightbox(null)}
          style={{ position:'fixed', inset:0, zIndex:1000, background:'rgba(0,0,0,0.93)', backdropFilter:'blur(20px)', display:'flex', alignItems:'center', justifyContent:'center', cursor:'zoom-out', animation:'dLightbox .2s ease-out' }}>
          <img src={lightbox} alt="screenshot" style={{ maxWidth:'90vw', maxHeight:'90vh', objectFit:'contain', borderRadius:14 }} />
        ...
        <div style={{ borderBottom:'1px solid rgba(255,255,255,0.05)', background:'rgba(6,6,10,0.8)', backdropFilter:'blur(20px)', padding:'22px 32px' }}>
          <div style={{ maxWidth:1200, margin:'0 auto', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:14 }}>
            <div style={{ display:'flex', alignItems:'center', gap:14 }}>
              ...
              <div>
                <h1 style={{ fontSize:22, fontWeight:800, margin:0, background:'linear-gradient(135deg,#fff 0%,#a1a1aa 100%)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent' }}>Deposit Requests</h1>
                <p style={{ fontSize:13, color:'#52525b', margin:'2px 0 0 0', fontWeight:500 }}>Review and approve artist deposits</p>
              </div>
            </div>
            <div style={{ display:'flex', gap:10, alignItems:'center' }}>
              {pending > 0 && (
                <div style={{ display:'flex', alignItems:'center', gap:7, padding:'7px 14px', background:'rgba(245,158,11,0.08)', border:'1px solid rgba(245,158,11,0.2)', borderRadius:10 }}>
                  <div style={{ width:6, height:6, borderRadius:'50%', background:'#fbbf24', animation:'dPulseDot 1.5s ease-in-out infinite' }} />
              ...
              <button onClick={() => load(true)} disabled={refreshing}
                ...
                <RefreshCw size={13} style={{ animation: refreshing ? 'dSpin 1s linear infinite' : 'none' }} /> Refresh
              </button>
            </div>
          </div>
        </div>
        ...
        <div style={{ maxWidth:1200, margin:'0 auto', padding:'24px 32px 60px' }}>
          ...
          {filtered.length === 0 ? (
            <div style={{ display:'flex', flexDirection:'column', alignItems:'center', padding:'80px 0', gap:12, background:'rgba(255,255,255,0.02)', border:'1px solid rgba(255,255,255,0.05)', borderRadius:20 }}>
              ...
              <p style={{ color:'#52525b', fontSize:15, fontWeight:600, margin:0 }}>No {filter} requests</p>
            </div>
          ) : (
            <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
              {filtered.map((req, idx) => {
                ...
                return (
                  <div key={req.id} style={{ background:'rgba(255,255,255,0.03)', border:'1px solid rgba(255,255,255,0.07)', borderRadius:18, overflow:'hidden', animation:`dFadeInUp .35s ease-out ${idx * 0.04}s both` }}>
                    ...
                    <div style={{ padding:'20px 24px', display:'flex', alignItems:'center', gap:16, flexWrap:'wrap' }}>
                      ...
                      <div style={{ flex:1, minWidth:180 }}>
                        ...
                        <p style={{ fontSize:12, color:'#52525b', margin:'2px 0 0 0' }}>{req.user?.email} · {req.user?.role}</p>
                      </div>
                      ...
                      <div style={{ textAlign:'center', minWidth:100 }}>
                        ...
                        <p style={{ fontSize:20, fontWeight:800, color:'#34d399', margin:0 }}>₹{req.amount.toLocaleString('en-IN')}</p>
                      </div>

                      {/* UTR */}
                      <div style={{ minWidth:140 }}>
                        <p style={{ fontSize:10, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.08em', color:'#3f3f46', margin:'0 0 3px 0' }}>UTR</p>
                        <p style={{ fontSize:13, fontWeight:600, color:'#a1a1aa', margin:0 }}>{req.utrNumber || '—'}</p>
                      </div>

                      {/* Date */}
                      <div style={{ minWidth:110 }}>
                        <p style={{ fontSize:10, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.08em', color:'#3f3f46', margin:'0 0 3px 0' }}>Date</p>
                      ...
                      {req.screenshotUrl && (
                        <div>
                          <button onClick={() => setLightbox(req.screenshotUrl)}
                            style={{ display:'flex', alignItems:'center', gap:6, padding:'8px 14px', background:'rgba(168,85,247,0.08)', border:'1px solid rgba(168,85,247,0.18)', borderRadius:10, color:'#c084fc', fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>
                            🖼 View SS
                          </button>
                        </div>
                      )}
                      ...
                      {req.status === 'pending' && (
                        <div style={{ display:'flex', gap:8 }}>
                          <button onClick={() => approve(req.id)} disabled={isProc}
                            ...
                            {isProc ? <Loader2 size={13} style={{ animation:'dSpin 1s linear infinite' }} /> : <CheckCircle size={13} />}
                            Approve
                          </button>
                    ...
                    {isReject && (
                      <div style={{ borderTop:'1px solid rgba(255,255,255,0.06)', padding:'16px 24px', background:'rgba(239,68,68,0.03)', display:'flex', gap:10, alignItems:'center', flexWrap:'wrap' }}>
                        <input placeholder="Reason for rejection (optional)" value={rejectNote}
                          onChange={e => setRejectNote(e.target.value)} className="d-input"
                        ...
                        <button onClick={() => reject(req.id)} disabled={isProc}
                          ...
                          {isProc ? <Loader2 size={13} style={{ animation:'dSpin 1s linear infinite' }} /> : null}
                          Confirm Reject
                        </button>
