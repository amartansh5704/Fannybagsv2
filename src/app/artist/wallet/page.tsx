    ...
    return (
      <ArtistLayout>
        ...
        <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', minHeight:'100vh', background:'#06060a' }}>
          <Loader2 style={{ animation:'wSpin 1s linear infinite', color:'#a855f7', width:32, height:32 }} />
  ...
  return (
    <ArtistLayout>
      <style jsx global>{`
        ...
        @keyframes wFadeInUp   { from{opacity:0;transform:translateY(14px)} to{opacity:1;transform:translateY(0)} }
        @keyframes wFadeInDown { from{opacity:0;transform:translateY(-10px)} to{opacity:1;transform:translateY(0)} }
        @keyframes wGradShift  { 0%{background-position:0% 50%} 50%{background-position:100% 50%} 100%{background-position:0% 50%} }
        @keyframes wPulseDot   { 0%,100%{opacity:1;transform:scale(1)} 50%{opacity:.4;transform:scale(1.4)} }
        @keyframes wPop        { 0%{transform:scale(.9);opacity:0} 60%{transform:scale(1.05)} 100%{transform:scale(1);opacity:1} }

        dialog::backdrop { background:rgba(0,0,0,0.85); backdrop-filter:blur(14px); }
        dialog { border:none; padding:0; background:transparent; }
        dialog[open] { animation:wFadeInUp 0.3s ease-out; }

        .w-input::placeholder { color:#3f3f46; }
        .w-input:focus { outline:none; border-color:rgba(168,85,247,0.4); box-shadow:0 0 0 3px rgba(168,85,247,0.08); }
        .w-tx-scroll::-webkit-scrollbar { width:4px; }
        .w-tx-scroll::-webkit-scrollbar-thumb { background:rgba(255,255,255,0.06); border-radius:2px; }
      ...
      <div style={{ minHeight:'100vh', background:'#06060a', color:'#fff', position:'relative', overflow:'hidden', fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif' }}>
        ...
        <div style={{ position:'fixed', inset:0, pointerEvents:'none', overflow:'hidden', zIndex:0 }}>
          <div style={{ position:'absolute', top:'-80px', right:'-40px', width:'500px', height:'500px', background:'radial-gradient(circle,rgba(168,85,247,0.07) 0%,transparent 70%)', borderRadius:'50%' }} />
          <div style={{ position:'absolute', bottom:'-60px', left:'-60px', width:'400px', height:'400px', background:'radial-gradient(circle,rgba(236,72,153,0.05) 0%,transparent 70%)', borderRadius:'50%' }} />
        ...
        <div style={{ position:'relative', zIndex:1, borderBottom:'1px solid rgba(255,255,255,0.05)', background:'rgba(6,6,10,0.8)', backdropFilter:'blur(20px)', animation:'wFadeInDown 0.4s ease-out' }}>
          <div className="w-header-pad" style={{ padding:'22px 32px', maxWidth:1100, margin:'0 auto', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:14 }}>
            ...
            <button onClick={() => loadAll(true)} disabled={refreshing}
              ...
              <RefreshCw size={13} style={{ animation: refreshing ? 'wSpin 1s linear infinite' : 'none' }} /> Refresh
            </button>
          </div>
        </div>
        ...
        <div className="w-page-pad" style={{ position:'relative', zIndex:1, padding:'28px 32px 60px', maxWidth:1100, margin:'0 auto' }}>
          ...
          {depositReqs.length > 0 && (
            <div style={{ marginBottom:28, animation:'wFadeInUp 0.4s ease-out 0.15s both' }}>
          ...
          {withdrawReqs.length > 0 && (
            <div style={{ marginBottom:28, animation:'wFadeInUp 0.4s ease-out 0.18s both' }}>
          ...
          {/* Transaction history */}
          <div style={{ animation:'wFadeInUp 0.4s ease-out 0.2s both' }}>
            ...
            {filteredTx.length === 0 ? (
              ...
              <div className="w-tx-scroll" style={{ display:'flex', flexDirection:'column', gap:8 }}>
      ...
      <dialog ref={depositDialogRef} style={{ width:480, maxWidth:'95vw', borderRadius:24, overflow:'hidden', boxShadow:'0 25px 80px rgba(0,0,0,0.7)' }}>
        <div style={{ background:'#0f0f14', border:'1px solid rgba(255,255,255,0.08)', borderRadius:24, overflow:'hidden' }}>
          ...
          <div style={{ padding:24, maxHeight:'65vh', overflowY:'auto' }} className="w-tx-scroll">
            {submitDone ? (
              ...
              <div>
                ...
                <input type="number" placeholder="e.g. 1000" value={depositAmt} onChange={e => setDepositAmt(e.target.value)} className="w-input"
              ...
              <div>
                ...
                <input placeholder="e.g. 425612345678" value={utrNumber} onChange={e => setUtrNumber(e.target.value)} className="w-input"
                ...
                {ssUrl ? (
                  ...
                  <div onClick={() => ssInputRef.current?.click()}
                    ...
                    onMouseEnter={e => { if (!ssUploading) e.currentTarget.style.borderColor='rgba(168,85,247,0.3)' }}
                    onMouseLeave={e => { e.currentTarget.style.borderColor='rgba(255,255,255,0.12)' }}>
                    {ssUploading ? <Loader2 size={22} style={{ animation:'wSpin 1s linear infinite', color:'#c084fc' }} /> : <Upload size={22} color="#52525b" />}
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
                  ...
                  <button onClick={submitDepositRequest} disabled={submitting || ssUploading}
                    ...
                    {submitting ? <><Loader2 size={14} style={{ animation:'wSpin 1s linear infinite' }} /> Submitting...</> : '✓ Submit Request'}
                  </button>
                </div>
              </div>
            )}
              ...
              <>
                ...
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
                  ...
                  <button onClick={submitWithdrawal} disabled={payingOut}
                    ...
                    {payingOut ? <><Loader2 size={14} style={{ animation:'wSpin 1s linear infinite' }} /> Submitting...</> : '🏦 Request Withdrawal'}
                  </button>
                </div>
              </>
            )}
